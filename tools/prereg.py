...
def main():
    ...
    ap.add_argument("--build-commit",
                    help="the commit whose CI run produced this SDF, when that "
                         "is not HEAD. Recorded separately so provenance is "
                         "not a matter of trusting that nothing moved.")
    ...
    # The path as a READER can use it, not as this machine saw it. An absolute
    # path into somebody's scratch directory is provenance nobody can follow,
    # so a path that runs through the build artifacts is cut back to the part
    # that names the run.
    rel = os.path.relpath(args.sdf, ROOT)
    if rel.startswith(".."):
        i = args.sdf.find("runs/")
        rel = args.sdf[i:] if i >= 0 else args.sdf
    head = dict(sdf=rel,
                corner=corner, commit=commit(),
                build=(f" The SDF is from the CI build of commit "
                       f"`{args.build_commit}`, whose RTL is identical to this "
                       f"one." if args.build_commit else ""),
