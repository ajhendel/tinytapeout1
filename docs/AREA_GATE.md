...
**This table used to be typed here by hand and it had gone stale in three ways
by 2026-08-28.** It is now generated by `tools/slew_range.py` from the build's
own static timing reports and gated in CI, because a table that is typed goes
stale silently and this one had. It said the worst structure was calibration
ring 5 at 1.320 ns; ring 5 is now at 1.008 and the worst is the characterization
merge node. It said the pins past 1.5 ns were all on the `rst_n` chain; they are
now all on a fabric site's `live` control chain. The only row that survived
unchanged is the TDC, which still reports nothing at all.

Measured on the 2026-08-28 build, worst corner (max_ss_100C_1v60):

| structure | worst slew at ss | of the 1.5 ns limit | pin |
|---|---|---|---|
| characterization paths | 1.318 ns | 88 % | `u_char.merge_out.../A` |
| fabric sites | 1.136 ns | 76 % | `sites[12].u_site.route_mux.u/A0` |
| calibration strip | 1.008 ns | 67 % | `u_calib.ro5.stage[5].u.drv1.g1.u/Z` |
| TDC | none reported | | |

The merge node is the one to watch. It is the single node every characterization
reading passes through, it is at 88 percent of the range at the slow corner, and
it got there because twenty tri-state elements share it. It is inside the table
and it does not have much of the table left.
...
**Seven pins in the whole design do exceed 1.5 ns, and none of them is ours.**
On the 2026-08-28 build all seven are on the chain OpenROAD grew from
`sites[0].u_site.live`, a per-site control signal, repaired with
`clkdlybuf4s25_1`, a DELAY buffer, giving 1.617 ns at max_ss. It is a slow
corner effect only: 1.018 ns at tt and 0.764 ns at ff, both inside the range.
Nothing is quoted against a control signal, so `tools/slew_range.py` reports
this group and does not gate it. On the 2026-08-27 build the same seven pins
were on the `rst_n` chain instead, at 1.813 ns, which is how much this moves
between builds and why it is generated rather than typed.
...
### Built, 2026-08-27, and rebuilt 2026-08-28 with the instrument fixes

| quantity | projected | 2026-08-27 | **2026-08-28** |
|---|---|---|---|
| standard cell area | | 63,627 um2 | |
| placed instances | | | **54,874** |
| utilization in 6x2 | 33.3 %, or 30.5 % corrected | 28.2 % | **28.4 %** |
| setup WNS, worst corner | | +5.60 ns | **+7.10 ns** |
| hold WNS, worst corner | | +0.108 ns | **+0.108 ns** |
| routing DRC / antenna | | 0 / 0 | **0 / 0** |
| total power, nominal | | 3.59 mW | |

The 2026-08-28 column is the build that carries the Gray coarse counter, the
causal ring kill, the fifth sampling branch and the ring tap buffer. **Every
required gate passes on it**, including the four instrument gates, at all nine
corners. The whole instrument round cost 0.2 points of utilization.
