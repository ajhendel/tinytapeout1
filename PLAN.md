...
- **C. Geometry replicas** (small). 2–3 logically identical paths with mirrored/spread placement, for netlist-vs-geometry transfer. **Partly built 2026-08-27 as calibration rings 0, 6 and 7, three identical circuits. Placement CANNOT be requested in the Tiny Tapeout flow, so what ships is three identical rings plus tools/check_placement.py, which reports the separation the build actually achieved. A clustered build cannot support a spatial claim, and the report says so rather than the paper saying otherwise.**
...
- **Phase 5. Tapeout two.** Tier-3 custom primitives (transistor-internal faults, current-starved cells, dynamic nodes, analog taps, real p-bit cells), designed from tapeout-one evidence, on an analog-capable shuttle slot.
