...
## Layout
    ...
    predictions/
      ...
      RULES.md                   the analysis rules: exclusions, ambiguity,
                                 repeats, multiple comparisons, the die split
      generated/                 tools/prereg.py output, from one named build
...
- `RULES.md` is the half that cannot be generated: how a raw capture becomes a
  delay, which readings are excluded and why, what happens to an ambiguous one,
  the repeat counts and the dither precondition they rest on, the multiple
  comparison correction for the search, and the die split. Those are judgements
  and every one of them was written while the answer was unknown, which is the
  only time writing them is worth anything. **Committed 2026-08-28.**
- The numbers are generated by `tools/prereg.py` from the shipped build's own
  SDF, into `predictions/generated/`, recording the artifact and the commit they
  came from. They are generated rather than typed because a value copied by hand
  from a report can be copied wrongly, copied from the wrong corner, or quietly
...
Scaffold only. Nothing is predicted yet, because the RTL is not frozen and
predicting the behaviour of a design that is still moving is a way of predicting
nothing. The predictions are written in WP5, after WP4 freezes the RTL, and
before the shuttle deadline in docs/TT_LOGISTICS.md.
...
- **The isolation pair.** Paths 15 and 16 differ only in whether each drive
